...
public class RegistrationService {
	...
	/**
	 * Registers a new user with the specified role and redirects them to the
	...
	public RedirectView registerAdmin(User user) {
		...
		logger.info("🔎 User '{}' retrieved from DB after save. Assigned roles: {}", savedUser.get().getUsername(),
				savedUser.get().getRoles());

		// Step 8: Verify if the user has ADMIN role
		boolean isAdminAssigned = savedUser.get().getRoles().stream().anyMatch(role -> role.getName().equals("ADMIN"));
	...
	private boolean isAuthenticatedUserAdmin() {
		...
		if (principal instanceof UserDetails userDetails) {
			...
			boolean isAdmin = user.isPresent()
					&& user.get().getRoles().stream().anyMatch(role -> role.getName().equals("ADMIN"));
