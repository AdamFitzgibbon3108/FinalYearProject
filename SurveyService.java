...
public class SurveyService {
	...
	public void saveSurveyResponses(List<SurveyResponse> responses, String username) {
		...
		for (SurveyResponse response : responses) {
			...
			Optional<SurveyQuestionOptions> optionOpt = optionsRepository
					.findById(response.getSelectedOption().getId());
	...
	public void updateUserRecommendations(String username, List<String> categories) {
		...
		if (userOptional.isPresent()) {
			...
			logger.info(
					"Updated recommended security categories for user: " + username + " -> " + recommendationsString);
		} else {
	...
	private List<String> getTopCategories(Map<String, Integer> categoryScores) {
		return categoryScores.entrySet().stream().sorted((a, b) -> b.getValue().compareTo(a.getValue())).limit(3)
